...
class StackedLSTM_new(nn.Module):
    def __init__(self, 
                 num_endog_features, 
                 num_exog_features, 
                 hidden_size=32, 
                 ...
                 output_size=1, 
                 dropout=0.0):
        super(StackedLSTM_new, self).__init__()

        # LSTM layer for endogenous features (time-series data)
        self.lstm = nn.LSTM(num_endog_features, hidden_size, num_layers, batch_first=True, dropout=dropout)

        # Fully connected layer to combine LSTM output with exogenous features
        self.fc = nn.Linear(hidden_size + num_exog_features, output_size)

    def forward(self, x_endog, x_exog):
        # Pass only endogenous features through LSTM
        lstm_out, _ = self.lstm(x_endog)  # LSTM output of shape (batch_size, seq_len, hidden_size)
        
        # Take only the last time step output from LSTM
        lstm_out_last = lstm_out[:, -1, :]  # Shape: (batch_size, hidden_size)

        # Concatenate LSTM output with exogenous features (policy data)
        combined = torch.cat((lstm_out_last, x_exog), dim=1)  # Shape: (batch_size, hidden_size + num_exog_features)
